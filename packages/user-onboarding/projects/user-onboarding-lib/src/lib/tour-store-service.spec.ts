...
import { of } from "rxjs";
import { argFn, argFunction, commandDS, commandDT, commandLS, commandLT, commandSS, commandST } from "../mocks/tourStore.mocks";
import { parameterDS, parameterDT, parameterLS, parameterLT, parameterSS, parameterST, sessionId } from "../mocks/tourStore.mocks";
...
describe('TourStoreServiceService', () => {
    ...
    it('Service injected via inject([TourStoreServiceService] and TestBed.inject(TourStoreServiceServiceshould be the same instance) ',
        inject([TourStoreServiceService], (injectedService: TourStoreServiceService) => {
            expect(injectedService).toBeTruthy();
            expect(injectedService).toBe(service);
        }));
    it('Register Save tour Command', () => {
        const result = service.registerSaveTourCommand(commandST);
    ...
    it('Should get sessionId', inject([LOCAL_STORAGE], (storage: StorageService) => {
