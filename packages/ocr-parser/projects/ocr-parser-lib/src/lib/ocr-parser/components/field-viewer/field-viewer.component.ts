...
export class FieldViewerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.dataService.$getUpdatedClauseValue.subscribe((text: string) => {
        const field = this.data.fieldData?.find(
          (item: FieldData) => item.isSelected,
        );
        if (field) {
          field.value = text;
          field.score = 100;
  ...
  onEmitFieldData(data: FieldData, isScroll = false) {
    ...
    if (field) field.isSelected = false;
