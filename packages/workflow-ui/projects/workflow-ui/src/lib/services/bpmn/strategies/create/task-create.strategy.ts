...
export class CreateTaskStrategy implements CreateStrategy<ModdleElement> {
  ...
  getInputFromPrev(
    ...
    const prevGateways = node.prev.filter(n => n.element.constructor.name === 'GatewayElement');
    if (prevGateways.length) {
      return node.element.id?.split('_').includes('true') ? prevGateways.map(e => (e.element as GatewayElement).elseOutGoing) : prevGateways.map(e => e.outgoing);
    } else {
