...
@Injectable()
export class CreateEGatewayStrategy implements CreateStrategy<ModdleElement> {
  constructor(
  ...
  execute(
    ...
    (element as EGatewayElement).elseOutGoing = `Flow_${this.utils.uuid()}`;
    (element as EGatewayElement).default = `Flow_${this.utils.uuid()}`;
