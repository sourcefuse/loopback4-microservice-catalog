...
import {TaskType, WorkflowManagerExtended} from '@sourceloop/task-service';
import {CamundaService} from '../services/camunda.service';

@bind({scope: BindingScope.TRANSIENT})
export class BpmnProvider implements Provider<WorkflowManagerExtended> {
  constructor(
    @service(CamundaService)
    private readonly camunda: CamundaService,
  ) {}
  value(): WorkflowManagerExtended {
    return {
      ...
      completeWorkflowTask: async (
        ...
        if (type === TaskType.User) {
          await this.camunda.completeUserTask(id, variables);
        }
      },
    };
  }
