...
describe('Message Recipient Controller', () => {
    ...
    it('gives status 200 and an array of message recipients', async () => {
        ...
        testlab_1.expect(reqToAddMessageRecipient.status).to.be.equal(200);
    ...
    it('updates message recipient successfully using PATCH request', async () => {
        ...
        const response = await client
            ...
            .set('authorization', `Bearer ${token}`)
            .expect(200);
        testlab_1.expect(response.body).to.have.properties(['channelId']);
        testlab_1.expect(response.body.channelId).to.be.equal('updated_channel');
    });
...
//# sourceMappingURL=message-recipient.controller.acceptance.js.map