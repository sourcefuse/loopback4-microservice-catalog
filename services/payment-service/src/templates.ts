...
export const stripeCreateTemplate = `<html>
  ...
  data-amount=
  {{orderAmount}}
  data-currency={{currency}} data-name="" data-description="Test Stripe" data-locale="auto" > </script>
  </form>
  </body>
  </html>`;
...
export const stripeSubscriptionCreateTemplate = `<html>
  ...
  <body>
  <h3>Welcome to Payment Gateway</h3>
  <form action="/transactions/charge?method=stripe&subscriptionId={{subscriptionId}}" method="POST">
  <script src="//checkout.stripe.com/v2/checkout.js" class="stripe-button" data-key=
  {{publishKey}}
  data-amount=
  {{orderAmount}}
