  ...
  export class UserOperationsService {
    constructor(
      ...
      @repository(UserViewRepository) readonly userViewRepository: UserViewRepository,
      @repository(TenantRepository) readonly tenantRepository: TenantRepository,
      @repository(UserGroupRepository) readonly userGroupRepository:UserGroupRepository,
    ...
    async create(userDtoData: UserDto, tenantId: string): Promise<User> {
      ...
      const currentUser = await this.userRepository.findById(currentUserToken.id??'');
    ...
    async find(tenantId: string, filter?: Filter<UserView>, permissions?: string[]) {
    ...
    async updateById(userData: Omit<
        UserView,
