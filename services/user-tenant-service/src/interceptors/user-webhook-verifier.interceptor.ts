...
let DEFAULT_TIME_TOLERANCE = 5000;

export class UserWebhookVerifierProvider implements Provider<Interceptor> {
    constructor(
    ...
    async intercept<T>(
        ...
        const value: UserWebhookDTO = request.body;
        const TIMESTAMP_TOLERANCE = +(process.env.TIMESTAMP_TOLERANCE ?? DEFAULT_TIME_TOLERANCE);
        ...
        try {
            ...
            let hh = Math.abs(timestamp - Date.now());
