...
CREATE SEQUENCE main.auth_clients_id_seq;

CREATE  TABLE main.auth_clients ( 
	id                   integer  NOT NULL GENERATED {BY DEFAULT|ALWAYS} AS IDENTITY [(AS ~dataType~ |INCREMENT BY ~increment~ |MINVALUE ~minvalue~ |NO MINVALUE |MAXVALUE ~maxvalue~ |NO MAXVALUE |START WITH ~start~ |CACHE ~cache~ |CYCLE |OWNED BY ~tableDotColumn~ )] |GENERATED ALWAYS AS ( ~generation_expr~ ) [STORED] ,
...
CREATE  TABLE main.organization_roles ( 
	id                   uuid DEFAULT (md5(((random())::text || (clock_timestamp())::text)))::uuid NOT NULL  ,
	...
	deleted_by           uuid    ,
	deleted_on           timestamptz    ,
	CONSTRAINT organisation_roles_id PRIMARY KEY ( id )
 );
...
CREATE  TABLE main.user_tenant_prefs ( 
	id                   uuid DEFAULT (md5(((random())::text || (clock_timestamp())::text)))::uuid NOT NULL  ,
	config_key           varchar(100)  NOT NULL  ,
	config_value         jsonb  NOT NULL  ,
	created_on           timestamptz(35) DEFAULT CURRENT_TIMESTAMP NOT NULL  ,
...
CREATE TRIGGER roles_audit_trigger AFTER INSERT OR DELETE OR UPDATE ON main.roles FOR EACH ROW EXECUTE PROCEDURE logs.audit_trigger('ROLE_LOGS');

CREATE TRIGGER tenant_configs_audit_trigger AFTER INSERT OR DELETE OR UPDATE ON main.tenant_configs FOR EACH ROW EXECUTE PROCEDURE logs.audit_trigger('TENANT_CONFIG_LOGS');

CREATE TRIGGER tenants_audit_trigger AFTER INSERT OR DELETE OR UPDATE ON main.tenants FOR EACH ROW EXECUTE PROCEDURE logs.audit_trigger('TENANT_LOGS');

CREATE TRIGGER user_permissions_audit_trigger AFTER INSERT OR DELETE OR UPDATE ON main.user_permissions FOR EACH ROW EXECUTE PROCEDURE logs.audit_trigger('USER_PERMISSION_LOGS');

CREATE TRIGGER users_audit_trigger AFTER INSERT OR DELETE OR UPDATE ON main.users FOR EACH ROW EXECUTE PROCEDURE logs.audit_trigger('USER_LOGS');
