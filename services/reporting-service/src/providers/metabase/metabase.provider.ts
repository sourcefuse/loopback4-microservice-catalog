...
export class MetabaseProvider implements Provider<MetabaseReports> {
  constructor(
    @inject(AuthenticationBindings.CURRENT_USER) private user: DataObject<{role:string}>,
  ...
  value() {
    return {
      getQueryData: async (queryId: string, userRole: string) => {
        ...
        const sessionTokenFind = await this.sessionTokenRepository.findOne({where:{name:'metabaseToken'}});
        const date = new Date();
        const daysDiff = moment(date).diff(sessionTokenFind?.modifiedOn,'days');
        let tokenValue = sessionTokenFind?.sessionKey ?? '';
        if(sessionTokenFind === null || daysDiff > 13){
          await axios.post(`${metabaseBaseUrl}/api/session`,
          {
          username: `${metabaseUserName}`,
          password: `${metabasePassword}`
          })
          // eslint-disable-next-line  @typescript-eslint/no-explicit-any
          .then(function (respon: DataObject<{data:DataObject<{id:string}>}> | any) {
          tokenValue = respon.data.id;
          }).catch((err:unknown)=>{
            console.log(err);
          })
          if(sessionTokenFind !== null && daysDiff > 13){
          await this.sessionTokenRepository.updateById(sessionTokenFind?.id, {...sessionTokenFind,sessionKey:tokenValue});
          }else{
            const tokenData = {
              id: uuidv4() || '',
              name:'metabaseToken',
